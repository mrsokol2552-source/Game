...
PROBE_PREFIX = "[CombatPressureProbe]"
DEFAULT_FIXTURE = "Tests.PlayMode.FpsStressTests"
DEFAULT_METHOD = "OwnerTarget100v100_PathPressure_LogsCombatPressureProbe"
...
EXPECTED_PLAYER_UNITS = 100
EXPECTED_ENEMY_UNITS = 100
EXPECTED_TOTAL_UNITS = EXPECTED_PLAYER_UNITS + EXPECTED_ENEMY_UNITS
EXPECTED_MEASUREMENT = "ownerTarget100v100"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extract the latest 100v100 combat pressure diagnostic probe from Unity TestResults.xml."
    )
    parser.add_argument(
        "--xml",
        type=Path,
...
def evaluate_provisional_budget(result: ProbeResult) -> BudgetEvaluation:
    ...
    limits: dict[str, Any] = {
        ...
        "expected_player_units": EXPECTED_PLAYER_UNITS,
        "expected_enemy_units": EXPECTED_ENEMY_UNITS,
    ...
    if measurement != EXPECTED_MEASUREMENT:
        hard_failures.append(f"measurement={measurement!r}, expected {EXPECTED_MEASUREMENT!r}")

    check_exact_metric(metrics, "playerUnits", EXPECTED_PLAYER_UNITS, hard_failures)
    check_exact_metric(metrics, "enemyUnits", EXPECTED_ENEMY_UNITS, hard_failures)
    check_exact_metric(metrics, "totalUnits", EXPECTED_TOTAL_UNITS, hard_failures)

    path_pressure = metrics.get("pathPressure")
    if path_pressure is not True:
        hard_failures.append(f"pathPressure={path_pressure!r}, expected True")
    ...
    if not is_number(seconds):
        hard_failures.append("seconds metric is missing or not numeric")
    elif seconds > PROVISIONAL_MAX_DURATION_SECONDS:
        hard_failures.append(
            f"seconds={seconds:.3f} exceeds {PROVISIONAL_MAX_DURATION_SECONDS:.3f}"
        )
    ...
    total_path_builds = metrics.get("totalPathBuilds")
    if not is_number(total_path_builds):
        hard_failures.append("totalPathBuilds metric is missing or not numeric")
    elif total_path_builds <= 0:
        hard_failures.append(f"totalPathBuilds={total_path_builds} must be greater than zero")

    attack_events = metrics.get("attackEvents")
    if not is_number(attack_events):
        hard_failures.append("attackEvents metric is missing or not numeric")
    elif attack_events <= 0:
        hard_failures.append(f"attackEvents={attack_events} must be greater than zero")

    status = "failed" if hard_failures else "passed"
    return BudgetEvaluation(status, hard_failures, warnings, limits)


def check_exact_metric(
    metrics: dict[str, Any],
    key: str,
    expected: int,
    hard_failures: list[str],
    ...
    if not is_number(value):
        hard_failures.append(f"{key} metric is missing or not numeric")
    elif value != expected:
        hard_failures.append(f"{key}={value}, expected {expected}")
...
def format_summary(result: ProbeResult) -> str:
    ...
    fields = [
        ...
        f"playerUnits={metrics.get('playerUnits')}",
        f"enemyUnits={metrics.get('enemyUnits')}",
        f"totalUnits={metrics.get('totalUnits')}",
        f"frames={metrics.get('frames')}",
        f"avgFrameMs={metrics.get('avgFrameMs')}",
        f"maxFrameMs={metrics.get('maxFrameMs')}",
        f"attackEvents={metrics.get('attackEvents')}",
        f"totalPathBuilds={metrics.get('totalPathBuilds')}",
        f"pathResets={metrics.get('pathResets')}",
        f"maxPathBuildsFrame={metrics.get('maxPathBuildsFrame')}",
        f"maxPathCommandsFrame={metrics.get('maxPathCommandsFrame')}",
    ]
    return "CombatPressureProbe " + " ".join(fields)
...
def format_budget_summary(evaluation: BudgetEvaluation) -> str:
    fields = [
        ...
        f"expectedTotalUnits={evaluation.limits['expected_total_units']}",
    ]
