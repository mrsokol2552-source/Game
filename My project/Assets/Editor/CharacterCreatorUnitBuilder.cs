...
// [CODE-ID: EDITOR-CHARACTERCREATORUNITBUILDER]
// Logical block: Editor/CharacterCreatorUnitBuilder.
...
public static class CharacterCreatorUnitBuilder
{
    ...
    public static void BuildUnitAnimationSet()
    {
        ...
        string crouchIdlePath = Path.Combine(latestDir, "CrouchIdle.png").Replace('\\', '/');
        string crouchRunPath = Path.Combine(latestDir, "CrouchRun.png").Replace('\\', '/');
        ...
        Sprite[] deathSprites = File.Exists(Path.Combine(Application.dataPath, deathPath.Substring("Assets/".Length)))
            ...
            : Array.Empty<Sprite>();
        Sprite[] crouchIdleSprites = File.Exists(Path.Combine(Application.dataPath, crouchIdlePath.Substring("Assets/".Length)))
            ? AssetDatabase.LoadAllAssetRepresentationsAtPath(crouchIdlePath).OfType<Sprite>().ToArray()
            : Array.Empty<Sprite>();
        Sprite[] crouchRunSprites = File.Exists(Path.Combine(Application.dataPath, crouchRunPath.Substring("Assets/".Length)))
            ? AssetDatabase.LoadAllAssetRepresentationsAtPath(crouchRunPath).OfType<Sprite>().ToArray()
            : Array.Empty<Sprite>();
        ...
        animSet.CrouchIdleFrames = BuildDirectionalArray(crouchIdleSprites, framesPerDir);
        animSet.CrouchRunFrames = BuildDirectionalArray(crouchRunSprites, framesPerDir);
