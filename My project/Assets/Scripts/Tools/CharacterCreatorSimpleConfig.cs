...
// [CODE-ID: SCRIPTS-TOOLS-CHARACTERCREATORSIMPLECONFIG]
// Logical block: Scripts/Tools/CharacterCreatorSimpleConfig.
...
public class CharacterCreatorSimpleConfig : MonoBehaviour
{
    ...
    public string WeaponFolder = "Pistol1";
    public Color WeaponColor = Color.white;

    [Header("GunFire")]
    public bool UseGunFire = false;
    public string GunFireFolder = "GunFire1";
    ...
    public void ApplyToGenerator()
    {
        ...
        SetText(Generator.gunFireNameText, UseGunFire ? GunFireFolder : "None");
        ...
        {
            EnsureWeaponSlot();
            if (Generator.weapons != null && Generator.weapons.Length > 0)
            {
                var weapon = Generator.weapons[0];
                if (weapon.weaponGO != null)
                    weapon.weaponGO.name = WeaponFolder;
                if (weapon.spriteRenderer != null)
                {
                    weapon.spriteRenderer.enabled = true;
                    weapon.spriteRenderer.color = WeaponColor;
                }
                SetText(Generator.weaponNameText, WeaponFolder);
                SetText(Generator.weaponColorText, ColorToHex(WeaponColor));
            }
        }
        else
        {
        ...
        if (Generator.gunFireToggle != null)
            Generator.gunFireToggle.isOn = UseGunFire;
        if (Generator.gunFireRenderer != null)
            Generator.gunFireRenderer.enabled = UseGunFire;
    ...
    public void PreviewIdle()
    {
        ...
        if (UseWeapon)
        {
            ...
            {
                var weapon = Generator.weapons[0];
                SetLayerPreview(weapon.spriteRenderer, WeaponFolder, WeaponColor);
            }
        }
    ...
    public void ClearPreview()
    {
        ...
        if (Generator.weapons != null && Generator.weapons.Length > 0)
            ClearLayer(Generator.weapons[0].spriteRenderer);
    }
    ...
    private void EnsureWeaponSlot()
    {
        if (Generator == null) return;
        if (Generator.weapons == null || Generator.weapons.Length == 0)
        {
            var weaponGO = new GameObject("WeaponRenderer");
            weaponGO.transform.SetParent(Generator.transform, false);
            var sr = weaponGO.AddComponent<SpriteRenderer>();
            Generator.weapons = new[]
            {
                new SpritesheetGenerator.Weapon
                {
                    weaponGO = weaponGO,
                    animator = null,
                    spriteRenderer = sr
        ...
        if (weapon.spriteRenderer == null)
        {
            weapon.spriteRenderer = weapon.weaponGO.GetComponent<SpriteRenderer>();
            if (weapon.spriteRenderer == null)
                weapon.spriteRenderer = weapon.weaponGO.AddComponent<SpriteRenderer>();
        }
    }
}