...
namespace Game.Presentation.Pathfinding
{
    public class ProceduralEnvironment : MonoBehaviour
    {
        ...
        private void StartStreaming()
        {
            ...
            {
                // Reconfigure background grid after conversion so cell size matches square tiles.
        ...
        {
            // Enqueue active (visible) chunks first.
            ...
            // Then enqueue prefetch ring (outside active area).
        ...
        private void GenerateStreamChunk(Vector2Int coord)
        {
            ...
            if (UseBackgroundTilemap && _background != null)
            {
                ...
                if (bgBounds.size.x > 0 && bgBounds.size.y > 0)
                    FillStreamBackgroundChunk(state, rng);
            }
            ...
            if (StreamIncludeProps || StreamIncludeTrees)
            {
                ...
                if (useBackgroundForProps)
                    PlaceStreamPropsAndTreesBackground(state, rng);
                else
        ...
        private void FillStreamBackgroundChunk(StreamChunkState state, System.Random rng)
        {
            ...
            bool useWaterMask = UseWaterBiome && _backgroundWaterMask != null && _backgroundMaskWidth > 0 && _backgroundMaskHeight > 0;
            ...
            for (int y = 0; y < height; y++)
            {
                ...
                for (int x = 0; x < width; x++)
                {
                    ...
                    bool isWater = false;
                    bool isWaterHole = false;
                    bool isRock = false;
                    if (useWaterMask && col >= 0 && row >= 0 && col < _backgroundMaskWidth && row < _backgroundMaskHeight)
                    {
                        int globalIdx = (row * _backgroundMaskWidth) + col;
                        isWater = _backgroundWaterMask[globalIdx];
                        isWaterHole = !isWater && IsMaskHole(_backgroundWaterMask, _backgroundMaskWidth, _backgroundMaskHeight, col, row);
                        if (isWaterHole) isWater = true;
                        isRock = !isWater && _backgroundRockMask != null
                            && _backgroundRockMask.Length == _backgroundWaterMask.Length
                            && _backgroundRockMask[globalIdx];
                    }
                    bool isWaterInterior = isWater && (isWaterHole || (useWaterMask && IsMaskInterior(_backgroundWaterMask, _backgroundMaskWidth, _backgroundMaskHeight, col, row)));
                    ...
                    {
                        bool leftWater = col > 0 && IsWaterCell(_backgroundWaterMask, _backgroundMaskWidth, _backgroundMaskHeight, col - 1, row);
                        bool bottomWater = row > 0 && IsWaterCell(_backgroundWaterMask, _backgroundMaskWidth, _backgroundMaskHeight, col, row - 1);
                        bool rightWater = col < _backgroundMaskWidth - 1 && IsWaterCell(_backgroundWaterMask, _backgroundMaskWidth, _backgroundMaskHeight, col + 1, row);
                        bool topWater = row < _backgroundMaskHeight - 1 && IsWaterCell(_backgroundWaterMask, _backgroundMaskWidth, _backgroundMaskHeight, col, row + 1);
        ...
        // [PENV-09]
        // Background-grid tree/prop placement using direct mask checks to keep land, water,
        // and rock decorations separated.
        private void PlaceStreamPropsAndTreesBackground(StreamChunkState state, System.Random rng)
        {
            if (_background == null) return;
            var bgBounds = state.BackgroundBounds;
            if (bgBounds.size.x <= 0 || bgBounds.size.y <= 0) return;
            ...
            for (int y = 0; y < bgBounds.size.y; y++)
            {
                ...
                for (int x = 0; x < bgBounds.size.x; x++)
                {
                    ...
                    if (UseBackgroundTilemap && _backgroundGrid != null)
                    {
                        bool isWater = false;
                        bool isRock = false;
                        if (TryGetBackgroundMaskFlags(col, row, out isWater, out isRock))
                        {
                            if (isWater)
                                continue;
                            if (isRock)
                    ...
                    if (UseDirectWalkableUpdates && (placed || (propMap != null && propMap.GetTile(cell) != null)))
                    {
                        if (_hex != null)
                        {
                            var world = _background.GetCellCenterWorld(cell);
        ...
        {
            if (UseWaterBiome && _backgroundWaterMask != null && _backgroundMaskWidth > 0 && _backgroundMaskHeight > 0)
            {
                if (col >= 0 && row >= 0 && col < _backgroundMaskWidth && row < _backgroundMaskHeight)
                {
                    int idx = (row * _backgroundMaskWidth) + col;
                    bool isWater = _backgroundWaterMask[idx]
                        || IsMaskHole(_backgroundWaterMask, _backgroundMaskWidth, _backgroundMaskHeight, col, row);
                    bool isRock = !isWater && _backgroundRockMask != null
                        && _backgroundRockMask.Length == _backgroundWaterMask.Length
                        && _backgroundRockMask[idx];
                    if (isWater) return StreamBiomeType.Water;
                    if (isRock) return StreamBiomeType.Rock;
                }
                else if (TryGetBackgroundCellFlags(new Vector2Int(col, row), out bool isWater, out bool isRock))
                {
                    if (isWater) return StreamBiomeType.Water;
                    if (isRock) return StreamBiomeType.Rock;
                }
            ...
            out HashSet<Vector2Int> occupiedSet, out BlockBounds bounds)
        {
            ...
            if (UseFarViewBake)
            {
                ...
                ApplyFarViewState(false);
                // Invalidate baked content so we re-bake after regeneration (avoids stale "ground-only" far view).
                _farViewHasContent = false;
            }
