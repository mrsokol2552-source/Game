...
namespace SmallScaleInc.ZombieRural
{
    public class PlayerController : MonoBehaviour
    {
        ...
        private Transform flashlightTransform; // Store reference to FlashlightExample
        public SpriteRenderer flashlightRenderer; // Store reference to FlashlightExample
        public bool snapRotation = true; // Toggle for snapping or smooth rotation
        ...
        void Start()
        {
            ...
            // Get FlashlightExample child
            flashlightTransform = transform.Find("FlashlightExample");
            // Add a listener to detect when the toggle is clicked
        ...
        void Update()
        {
            ...
            // Rotate FlashlightExample to match player direction
            if (flashlightTransform != null)
            {
                    if (snapRotation)
                    {
                        flashlightTransform.rotation = Quaternion.Euler(0, 0, lastAngle);
                    }
                    else
                    {
                        flashlightTransform.rotation = Quaternion.Euler(0, 0, angle); // Smooth rotation without snapping
                    }
            }
        ...
        {
            if (isOn)
                StartCoroutine(FadeSprite(0f, 1f, 1.5f)); // Fade in (0 to 1 alpha in 1.5 sec)
            else
                StartCoroutine(FadeSprite(1f, 0f, 2f)); // Fade out (1 to 0 alpha in 2 sec)
        }

        // Coroutine to fade the sprite in or out
        private IEnumerator FadeSprite(float startAlpha, float endAlpha, float duration)
        {
            float elapsedTime = 0f;
            Color color = flashlightRenderer.color;

            while (elapsedTime < duration)
            {
                elapsedTime += Time.deltaTime;
                float newAlpha = Mathf.Lerp(startAlpha, endAlpha, elapsedTime / duration);
                flashlightRenderer.color = new Color(color.r, color.g, color.b, newAlpha);
                yield return null;
            }

            // Ensure final value is set
            flashlightRenderer.color = new Color(color.r, color.g, color.b, endAlpha);
        }
